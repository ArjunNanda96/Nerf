...
# torch.cudann bench -true benchmark


cuda = True
device = torch.device("cuda") if (cuda) else torch.device("cpu")
dataset = MVSDataset("./datasets/lego")
...
# Training variables
optimizer = torch.optim.Adam(model.parameters(), lr=5e-4) #1e-3 torch.scheduler cosine annealing
iterations = 1
rays_per_batch = 2**11
num_samples = 256
all_positions = []
